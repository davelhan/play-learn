...
(()=>{
  ...
  // Keep the existing draggable foot element/listeners, but detach it from the
  // old rigid leg so we can position it at the IK ankle target.
  ...
  Object.assign(svg.style,{
    position:'absolute',left:'0',top:'0',width:'270px',height:'420px',
  ...
  Object.assign(foot.style,{
    position:'absolute',width:'62px',height:'22px',margin:'0',top:'0',left:'0',
    zIndex:'12',transform:'none',transformOrigin:'50% 50%',cursor:'ew-resize',
    pointerEvents:'auto'
  });
  ...
  function solveKnee(fx,fy){
    ...
    // Keep target inside the reachable annulus without changing the slider value.
    ...
    // Knee bends opposite the direction of the swing foot, like a simple gait pose.
    const sign=dx>=0?1:-1;
  ...
  function apply(){
    ...
    // +22 cm is the neutral hanging pose. Positive values move the foot forward/right.
    const dx=clamp((value-22)*1.05,-74,74);
    const desiredX=HIP.x+dx;
    // Lift the swing foot as it travels away from neutral so the knee bends naturally.
    ...
    thigh.setAttribute('x2',p.x);thigh.setAttribute('y2',p.y);
    shin.setAttribute('x1',p.x);shin.setAttribute('y1',p.y);
    shin.setAttribute('x2',p.fx);shin.setAttribute('y2',p.fy);
    knee.setAttribute('cx',p.x);knee.setAttribute('cy',p.y);

    foot.style.left=`${p.fx-31}px`;
    foot.style.top=`${p.fy-9}px`;
    // Keep the sole roughly horizontal; a tiny toe-up cue helps at long reach.
    foot.style.transform=`rotate(${clamp(-dx*.035,-3,3)}deg)`;
  }

  // Main mission code updates slider.value during slider input and direct foot drag.
  // Schedule after those handlers so the visual always reflects the final state.
